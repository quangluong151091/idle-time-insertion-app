...
links.Timeline = function (container, options) {
	...
	this.options = {
		...
		'showCurrentTime': false, // show a red bar displaying the current time
		'showCustomTime': false, // show a blue, draggable bar displaying a custom time
		'showArrowTime': false, // show an arrow poitning to specific time
...
links.Timeline.prototype.repaintAxisHorizontal = function () {
	...
	if (hasAxis) {
		if (!axis.backgroundLine) {
			...
			backgroundLine.id = "targetLines";

			backgroundLine.style.display = "none";
			backgroundLine.className = "timeline-axis2";
			backgroundLine.style.position = "absolute";
			backgroundLine.style.left = options.targetLeft;
			backgroundLine.style.width = options.targetWidth;
			backgroundLine.style.border = "3px solid blue";
		...
		if (axis.backgroundLine) {
			axis.backgroundLine.style.top = size.axis.top + -2000 + "px";
			axis.backgroundLine.style.height = size.axis.height + 3000 + "px";
		}
	}
	else {
		if (axis.backgroundLine) {
...
links.Timeline.prototype.repaintGroups = function () {
	...
	if (!frame) {
		...
		frame.style.borderTop = "20px solid #ccc;";
	...
	for (var i = 0, iMax = Math.min(current, needed) ; i < iMax; i++) {
		var group = groups[i];
		var label = labels[i];
		var labelHTML = this.getGroupName(group);

		if (labelHTML === "Alarms") {
		  labelHTML=" <img src='img\/ic_alarm_30px_dark_grey.png'>" +labelHTML;
		}
		else if(labelHTML === "Triggers"){
			labelHTML="<img src='img\/ic_triggers_30px_wbg.png'> " +labelHTML;
		}
		else if (labelHTML === "Resets") {
			labelHTML="<img src='img\/ic_reset_30px_darkgrey.png' > "+labelHTML;
		}
		else if (labelHTML === "Service calls") {
		   labelHTML = "<img src='img\/ic_servicecall_30px_darkgrey.png' > "+labelHTML;
		}
		else if (labelHTML === "Machining Mode") {
			labelHTML="<img src='img/ic_machining_mode_30px_darkgrey.png'>" + labelHTML;
		}
	   else if (labelHTML == "Maintenance") {
		   labelHTML= "<img src='img\/ic_maintenance_30px_dark_grey.png' > "+ labelHTML;
		}
		else if (labelHTML.includes( "Video links")) {
		   labelHTML="<img src='img\/movie-icon-cluster.png' style='width:30px; height:30px;'>"+ labelHTML;
		}
		else if (labelHTML == "ManualOperation") {
			labelHTML = "<img src='img\/ic_manual_operation_30px_darkgrey.png'> "+ labelHTML;
		}
		else if (labelHTML == "Notes") {
			labelHTML = "<img src='img\/note_grey.png'> "+ labelHTML;
		}
		else {
		   labelHTML = labelHTML;
		}
	...
	for (var i = current; i < needed; i++) {
		...
		labelLine.style.borderTopWidth = "1px";
		labelLine.style.borderTopColor = "#4d4d4d";
		...
		itemLine.style.borderTopColor = "#4d4d4d";
...
links.Timeline.prototype.repaintArrowTime = function () {
	var options = this.options,
		dom = this.dom,
		size = this.size;
	if (!options.showArrowTime) {
		if (dom.arrowTime) {
			dom.contentTimelines.removeChild(dom.arrowTime);
			delete dom.arrowTime;
		}

		return;
	}
	if (!dom.arrowTime) {
		var arrowTime = document.createElement("DIV");
		arrowTime.className = "timeline-arrowtime";
		arrowTime.style.position = "absolute";
		arrowTime.style.bottom = "0px";
		arrowTime.style.height = "50px";
		arrowTime.id = "timeline-arrowtime";

		dom.contentTimelines.appendChild(arrowTime);
		dom.arrowTime = arrowTime;

	   // links.Timeline.addEventListener(dom.arrowTime, "click", showAlert);
		// initialize parameter
		this.arrowTime = new Date("October 13, 2000 00:00:00");//draws arrow out of timeline view on intiial draw
	}

	var x = this.timeToScreen(this.arrowTime),
		visible = (x > -size.contentWidth && x < 2 * size.contentWidth);
	dom.arrowTime.style.display = visible ? '' : 'none';
	dom.arrowTime.style.left = x-14 + "px";
	dom.arrowTime.title = "Time: " + this.arrowTime;
...
links.Timeline.prototype.setArrowTime = function (time) {
	this.arrowTime = new Date(time.valueOf());
	this.repaintArrowTime();
}
/**
...
links.Timeline.prototype.onMouseDown = function (event) {
	...
	function checkTime() { 
		...
		mousedowntimer = setInterval(function () { startTimer() }, 100);
		function startTimer() {
			mdcounter--;
			if (mdcounter <= 0) {clearInterval(mousedowntimer);mdcounter=2;};
		}
	}
	...
	params.customTime = (params.target === dom.customTime ||
		...
		this.customTime :
		undefined;
	params.arrowTime = (params.target === dom.arrowTime ||
		params.target.parentNode === dom.arrowTime) ?
		this.arrowTime :
		undefined;
	if (params.arrowTime) {
		var arrowTimeDate = new Date(params.arrowTime);
		var arrowTimeStart = new Date(arrowTimeDate.valueOf() - 5000);
		var arrowEndTime = new Date(arrowTimeDate.valueOf() + 5000);
		this.setVisibleChartRange(arrowTimeStart,arrowEndTime);
		this.applyRange(arrowTimeStart, arrowEndTime);
		this.trigger('rangechange');
		this.trigger('rangechanged');
		this.trigger('rangechange');
		this.trigger('rangechanged');
	}
	params.addItem = (options.editable && (event.ctrlKey||doubleClick));
	if (params.addItem) {
		console.log("additem")
...
links.Timeline.prototype.addItems = function (itemsData, preventRender) {
	newItemStart=itemsData[0].start;
	var timeline = this,
		items = this.items;
...
links.Timeline.prototype.getSelection = function () {
	...
	if (this.selection) {
		...
		this.unselectItem();this.selection = undefined;  ///////////////////////////////////////////////////////////////////////////////////////////////// bug fixed !!!!!!!!!!!!!!!!!!!!!!!
	return sel;
};
			...
			if (this.groups.hasOwnProperty(groupName)) {
				...
				while (i < iMax) {
					...
					if (neighbors > maxItems) {
						...
						// edit cluster content here
						var cluster;
						var group = item.group.content;
						var title;
						var alertType;
						
						title = 'Cluster containing ' + count +
                        ' items. Zoom in to see the individual items. Click to open list.'; 
                         alertType = " maintenence"
						
						var content = '<div  title="' + title + '" style="color:#918B87;font-size:10px;">' + wicon + count + '</div>';
